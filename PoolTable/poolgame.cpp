...
void PoolGame::simulateTimeStep(float timeStep)
{
    ...
    for(size_t i = 0; i < m_balls.size();++i)
    {
        for(size_t j = i+1;j < m_balls.size();++j)
        {
            ...
            {
                // Randomly swaps colour between balls.
                // Doesn't swap the colour with cue Balls though
                int changeColor = rand() % 2;
                if(changeColor == 0)
                {
                    QColor color1 = m_balls[i]->colour();
                    QColor color2 = m_balls[j]->colour();
                    CueBallDecorator* if1Cue = dynamic_cast<CueBallDecorator *> (m_balls[i]);
                    CueBallDecorator* if2Cue = dynamic_cast<CueBallDecorator *> (m_balls[j]);

                    if(if1Cue || if2Cue)
                    {
                        // Don't change colour;
                    }else{
                        m_balls[i]->setColour(color2);
                        m_balls[j]->setColour(color1);
                    }
                }
...
void PoolGame::setBall(std::vector<Ball *> balls)
{
    m_balls.clear();
    for(int i =0; i<balls.size(); i++)
    {
        m_balls.push_back(balls.at(i)->clone());
    }
}
